...
float Sobel_Depth_Horizontal(sampler2D texmap, vec2 texcoord)
{
	...
	//Is this affected by the change from row-major to column-major?***
	//Top Row
	sum -= clamp (0.967 - texture(texmap, vec2(texcoord.x - uPixelSize.x, texcoord.y + uPixelSize.y)).r, 0, 1);
	sum -= clamp (0.967 - texture(texmap, vec2(texcoord.x - uPixelSize.x, texcoord.y)).r * 2.0, 0, 1);
	sum -= clamp (0.967 - texture(texmap, vec2(texcoord.x - uPixelSize.x, texcoord.y - uPixelSize.y)).r;
...
float Sobel_Depth_Vertical(sampler2D texmap, vec2 texcoord)
{
	...
	//Top Row
	sum += clamp (0.967 -texture(texmap, vec2(texcoord.x - uPixelSize.x, texcoord.y + uPixelSize.y)).r, 0, 1);
	sum += clamp (0.967 -texture(texmap, vec2(texcoord.x, texcoord.y + uPixelSize.y)).r * 2.0, 0, 1);
	sum += clamp (0.967 -texture(texmap, vec2(texcoord.x + uPixelSize.x, texcoord.y + uPixelSize.y)).r, 0, 1);
