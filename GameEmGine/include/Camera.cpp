...
Camera::Camera(Size3D size, CAMERA_TYPE type):m_scale(1), m_projMat(1), m_objMat(1), m_cameraUpdate(true), m_position(new Quat{0,0,0,0})
{
...
bool Camera::update()
{
	if(m_cameraUpdate)
	{
		...
		m_objMat =  glm::inverse(m_transform.getRotationMatrix() *m_transform.getTranslationMatrix()) /** m_transform.getScaleMatrix()*/;
